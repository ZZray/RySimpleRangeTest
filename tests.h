...
void testBasicUsage()
{
    ...
    // 2. 测试 TestItem 添加
    RySimpleRangeTester<std::string>::TestItem item{
        "value3",
        "test3",
        "Manual test item",
        false
...
void testFailureScenarios()
{
    ...
    // 1. 添加预期会失败的测试
    tester.add(-1.0, "negative", "Should fail").expect("negative");

    // 2. 添加多个测试项，部分预期失败
    std::vector<RySimpleRangeTester<double>::TestItem> items = {
        {1.0, "positive1", "Should pass"},
        {-2.0, "negative2", "Should fail", true},
        {0.0, "zero", "Should pass"}
    };
    tester.add(items);

    // 3. 使用条件标记预期失败
    tester.expectIf([](const auto& item) {
        return item.item < 0;
        });
...
void testCustomTypes()
{
    ...
    tester.add(Point{ -1, 1 }, "invalid_point", "Should fail").expect("invalid_point");
